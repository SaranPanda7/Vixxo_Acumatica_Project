...
const Dashboard = () => {
  ...
  const getDashboardData = async () => {
    let response = await axios.get("https://dycp390e54.execute-api.us-east-2.amazonaws.com/dev/dashboard/", {
      headers: {
  ...
  if (user) {
    setInterval(getDashboardData, 50000);
  }

  const [serviceReqData, setServiceReqData] = useState();

  const getAppointmentData = async (a_id) => {
    try {
      let response = await axios.get(
        `https://dycp390e54.execute-api.us-east-2.amazonaws.com/dev/dashboard_sub_data/${a_id}`,
        {
          headers: {
            Authorization: `Bearer ${authtoken}`,
                        ...
                        <Box sx={{ margin: 1 }}>
                          <div className="">
                            <TableContainer component={Paper}>
                              <Table className="table-border" size="large">
                                <TableHead>
                                  <TableRow>
                                ...
                                <TableBody>
                                  {serviceReqData ? (
                                    ...
                                    <h5>No Scheduled Appointemnets</h5>
                                  )}
                                </TableBody>
                              </Table>
                            </TableContainer>
